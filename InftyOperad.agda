...
module InftyOperad where
  ...
  module _ {I : Type₀} (P : Poly I) where
    ...
    frame : {i : I} (w : W P i) (c : γ P i) → Type₀
    frame w c = (j : I) → Leaf P w j ≃ ρ P c j

    FillPoly : (F : {i : I} {w : W P i} {c : γ P i} → frame w c → Type₀) → Poly (Σ I (γ P))
    γ (FillPoly F) (i , c) = Σ (W P i) (λ w → Σ (frame w c) F)
