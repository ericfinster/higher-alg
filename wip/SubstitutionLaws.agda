...
module wip.SubstitutionLaws {ℓ} {I : Type ℓ} (P : Poly I) (R : PolyRel P) where
  ...
  subst-γ : (f : Ops P) (wα : InFrame P f)
    ...
    → InFrame P f
  subst-γ (i , f) (w , α) κ =
    subst w κ , λ j → α j ∘e (subst-lf-eqv w κ j) ⁻¹
  ...
  postulate
    ...
    subst-graft-frm-lem : {i : I} (w : W P i) (ψ : (j : I) → Leaf P w j → W P j)
      ...
      → Σ-fmap-r (λ i → λ { (l₀ , l₁) → subst-lf-from w κ i l₀ , subst-lf-from (ψ i (subst-lf-from w κ i l₀)) (λ g n → θ g (i , subst-lf-from w κ i l₀ , n)) j l₁ })
          (graft-leaf-from P (subst w κ) (λ j l → subst (ψ j (subst-lf-from w κ j l)) (λ g n → θ g (j , subst-lf-from w κ j l , n))) j (transport! (λ x → Leaf P x j) (subst-graft w ψ κ θ) l))
         == graft-leaf-from P w ψ j (subst-lf-from (graft P w ψ) (λ g n → ⊔-rec (κ g) (θ g) (graft-node-from P w ψ g n)) j l) 
  ...
  subst-assoc (nd (f , ϕ)) κ ν = 
    let (w , α) = κ (_ , f) (inl idp)
        ...
        α' j = α j ∘e (subst-lf-eqv w (λ g' n' → ν g' ((_ , f) , inl idp , n')) j) ⁻¹
  ...
  --   → (j : I) (l : Leaf P (subst (subst w κ) (λ g n → ν g (subst-nd-from w κ g n))) j)
  --   → subst-lf-from w (λ g n → subst-γ g (κ g n) (λ g₁ n₁ → ν g₁ (g , n , n₁))) j
  --                     (transport! (λ x → Leaf P x j) (subst-assoc w κ ν) l)
  --     == subst-lf-from w κ j (subst-lf-from (subst w κ) (λ g n → ν g (subst-nd-from w κ g n)) j l)
  ...
  --       α' k = α k ∘e (subst-lf-eqv w (λ g' n' → ν g' ((_ , f) , inl idp , n')) k) ⁻¹
  ...
  postulate
    ...
    subst-assoc-frm : {i : I} {f : Op P i}
      ...
      → (κ : (g : Ops P) → Node P w g → InFrame P g)
      → (ν : (g : Ops P) → Σ (Ops P) (λ h → Σ (Node P w h) (λ n → Node P (fst (κ h n)) g)) → InFrame P g)
      → (λ j → α j ∘e subst-lf-eqv w (λ g n → subst-γ g (κ g n) (λ g₁ n₁ → ν g₁ (g , n , n₁))) j ⁻¹) == 
        (λ j → (α j ∘e subst-lf-eqv w κ j ⁻¹) ∘e subst-lf-eqv (subst w κ) (λ g n → ν g (<– (subst-nd-eqv w κ g) n)) j ⁻¹)
    ...
    subst-nd-assoc : {i : I} {f : Op P i}
      ...
      → subst-nd-to w (λ g₁ n₁ → subst-γ g₁ (κ g₁ n₁) (λ g' n' → ν g' (g₁ , n₁ , n')))
  ...
  record SubstRelLaws (SRel : SubstRel) : Type ℓ where
    field
      ...
      subst-unit-l-rel : {i : I} {f : Op P i}
        ...
        → γ-rel SRel (i , f) (w , α) r (λ g _ → fst (η (SubstBiasedMgm SRel) g))
            == r [ R (i , f) ↓ pair= (subst-unit-l w) (subst-unit-l-frm w α) ]

      subst-unit-r-rel : {i : I} {f : Op P i}
        → (κ : (g : Ops P) → Node P (corolla P f) g → Op (P // R) g)
        → snd (κ (i , f) (inl idp)) == γ-rel SRel (i , f) (corolla P f , corolla-frm P f) (η-rel SRel (i , f)) (λ g n → fst (κ g n))
      ...
      subst-assoc-rel : {i : I} {f : Op P i}
        ...
        → let κ' g n = fst (κ g n)
          ...
          in γ-rel SRel (i , f) (w , α) r (λ g n → subst-γ g (κ' g n) (λ g₁ n₁ → ν' g₁ (g , n , n₁)))
             == γ-rel SRel (i , f) (subst-γ (i , f) (w , α) κ') (γ-rel SRel (i , f) (w , α) r κ')
                      (λ g n → fst (ν g (<– (subst-nd-eqv w κ' g) n)))
