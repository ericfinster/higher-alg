...
module wip.SubstitutionLaws {ℓ} {I : Type ℓ} (P : Poly I) (R : PolyRel P) where
  ...
  subst-graft (nd (f , ϕ)) ψ κ θ = 
    ...
    in graft-assoc P w ψ₀ ψ₁ ∙
       ap (graft P w) (Decor-== (Fr P)
          (λ j l → ih j l ∙ ap (subst (graft P (ϕ j (p j l)) (ψ' j l)))
  ...
  subst-graft-frm-lem : {i : I} (w : W P i) (ψ : (j : I) → Leaf P w j → W P j)
    ...
    → graft-lf-to P {w = subst w κ} (k , subst-lf-to {w = w} l₀ ,
        transport! (λ x → Leaf P (subst (ψ k x) (λ g n → θ g (k , x , n))) j)
                   (subst-leaf-from-to w κ k l₀) (subst-lf-to {w = ψ k l₀} l₁))
      == subst-lf-to {w = graft P w ψ} (graft-lf-to P {w = w} (k , l₀ , l₁)) [ (λ x → Leaf P x j) ↓ subst-graft w ψ κ θ ]
  subst-graft-frm-lem (lf i) ψ κ θ j .i idp l₁ = 
  ...
  subst-graft-frm-lem (nd (f , ϕ)) ψ κ θ j k (k₀ , p₀ , l₀) l₁ = 
    let (w , α) = κ (_ , f) (inl idp)
        ...
        lf₀ = <– (α k₀) p₀
        lf₁ = subst-lf-to {w = ϕ k₀ (p k₀ lf₀)} (transport! (λ x → Leaf P (ϕ k₀ x) k) (<–-inv-r (α k₀) p₀) l₀)
        lf₂ = subst-lf-to {w = ψ k (k₀ , p k₀ lf₀ , slf k k₀ lf₀ lf₁)}
                (transport! (λ x → Leaf P (ψ k (k₀ , fst x , snd x)) j) (pair= (<–-inv-r (α k₀) p₀) {!!}) l₁)
                
        g-assoc = graft-assoc-lf-ovr P w ψ₀ ψ₁ k₀ lf₀ k lf₁ j lf₂
