...
module PolyMonads where
  ...
  μρ-η : {I : Type₀} (M : Mnd I) {i : I} {c : γ M i}
         (δ : (p : ρ M c) → γ M (τ M p))
         (p : ρ M (μ M c δ)) → μp M δ (μρ-fst M δ p) (μρ-snd M δ p) == p

  -- Yeah, um, so these eta-expansion rules for the unit and multiplication
  -- are exactly what you want.  The problem is that, as rewrite rules, they
  -- are non-terminating!  (Uh, yeah, Agda doesn't even accept them ...)

  -- Worse, as it stands, they are incoherent!  You need that, applied to
  -- the canonical representations, they give the identity.  And if you add
  -- this, you'll have to prove it.  And so on ....
  ...
  module _ {I : Type₀} (M : Mnd I) (X : I → Type₀) where
    ...
    η-pb : (i : I-pb) → γ-pb i
    η-pb (i , x) = η M i , λ p → transport (λ p₀ → X (τ M p₀)) (ηp-η M i p) x
