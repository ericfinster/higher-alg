...
module Substitution {ℓ} {I : Type ℓ} {P : Poly I} (C : CartesianRel P) where
  ...
  flatten-frm-to (nd ((w , r) , κ)) j l = ? -- –> (α j) (substitute-lf-to w κ j l)
  
  flatten-frm-from (lf (i , f)) j p = (j , p , idp)
  flatten-frm-from (nd ((w , r) , κ)) j p = ? -- substitute-lf-from w κ j (<– (α j) p)
  ...
  bd-frame : {i : I} {f : Op P i}
    → (pd : W (P // R) (i , f))
    → (jg : Ops P) → Leaf (P // R) pd jg ≃ Node P (flatten pd) (snd jg)
  ...
  substitute-nd-to : {i : I} (w : W P i)
    ...
    → Σ (Ops P) (λ ke → Σ (Node P w (snd ke)) (λ n → Leaf (P // R) (κ ke n) jg))
  ...
  postulate
    ...
    substitute-nd-from-to : {i : I} (w : W P i)
      → (κ : (g : Ops P) → Node P w g → W (P // R) g) (jg : Ops P)
      → (t : Σ (Ops P) (λ ke → Σ (Node P w (snd ke)) (λ n → Leaf (P // R) (κ ke n) jg)))
  ...
  -- -- substitute-nd-to : {i : I} (w : W P i)
  -- --   → (κ : (g : Ops P) → Node P w g → W (P // R) c) (jg : Ops P)
  -- --   → Σ (Ops P) (λ ke → Σ (Node P w (snd ke)) (λ n → Leaf (P // R) (κ ke n) jg))
  -- --   → Node P (substitute w κ) (snd jg) 
