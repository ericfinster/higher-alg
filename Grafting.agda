...
module Grafting {ℓ} {I : Type ℓ} (P : Poly I) where
  ...
  --   → {j : I} (c : Op P j)
  --   → Node P (graft w ψ) c
  --   → Node P w c ⊔ Σ I (λ k → Σ (Leaf P w k) (λ l → Node P (ψ k l) c))
  -- graft-node-from' (lf i) ψ g n = inr (i , idp , n)
  -- graft-node-from' (nd (f ,  ϕ)) ψ .c (inl idp) = inl (inl idp)
  -- graft-node-from' (nd (f ,  ϕ)) ψ g (inr (j , p , n)) = 
  --   Coprod-elim
  --     {A = Node P (ϕ j p) g}
  --     {B = Σ I (λ k → Σ (Leaf P (ϕ j p) k) (λ l → Node P (ψ k (j , p , l)) g))}
  --     (λ n' → inl (inr (j , p , n')))
  ...
  --     (graft-node-from (ϕ j p) (λ k l → ψ k (j , p , l)) g n)
    ...
    --   → (ψ : ∀ j → Leaf P w j → W P j)
    --   → {j : I} (c : Op P j)
    --   → (n : Node P (graft w ψ) c)
    --   → graft-node-to w ψ c (graft-node-from' w ψ c n) == n
    -- graft-node-to-from' (lf i) ψ g n = idp
    -- graft-node-to-from' (nd (f ,  ϕ)) ψ .c (inl idp) = idp
    -- graft-node-to-from' (nd (f ,  ϕ)) ψ g (inr (j , p , n)) = {!(graft-node-from (ϕ j p) (λ k l → ψ k (j , p , l)) g n)!}
      -- Coprod-elim
      --   {A = Node P (ϕ j p) g}
      --   {B = Σ I (λ k → Σ (Leaf P (ϕ j p) k) (λ l → Node P (ψ k (j , p , l)) g))}
      --   {C = λ _ → graft-node-to (nd (f ,  ϕ)) ψ g (graft-node-from' (nd (f ,  ϕ)) ψ g (inr (j , p , n))) == inr (j , p , n)}
      --   {!!}
      --   {!!}
      --   (graft-node-from (ϕ j p) (λ k l → ψ k (j , p , l)) g n)
